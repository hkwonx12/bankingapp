...
function CreateInvestmentWithdrawForm({getStatementData}) {
    ...
    const handleSubmit = async (event) => {
        ...
        const url = "http://localhost:8000/api/investment_account";
        const withdrawFormData = {...formData, amount: -Math.abs(formData.amount)}
        const fetchConfig = {
            method: "PUT",
    ...
    <div className="w-full max-w-xs">
      <div className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        <div className="mb-4">
          <h1 className="block text-gray-700 text-sm font-bold mb-2" > Criossant United Bank</h1>
          <form onSubmit={handleSubmit} id="create-deposit-form">
            ...
            <button className="py-2 px-4  bg-purple-600 hover:bg-purple-700 focus:ring-purple-500 focus:ring-offset-purple-200 text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-lg ">Withdraw</button>
            </div>
          </form>
        </div>
      </div>
...
export default CreateInvestmentWithdrawForm;
