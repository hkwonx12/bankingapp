...
class InvestmentAccountRepository:
    def create_investment_account(
            self, investment_account: InvestmentAccountIn, user_id: int):
    ...
    def get_one_investment_account(
            self, owner_id: int) -> InvestmentAccountOutWithDetails:
    ...
    def get_all_investment_accounts(
            self, account_data) -> List[InvestmentAccountOutWithDetails]:
        with pool.connection() as conn:
            with conn.cursor() as db:
                result = db.execute(
                    """
    ...
    def investment_account_in_to_out(
            self, id: int, investment_account: InvestmentAccountOutWithDetails):
        old_data = investment_account.dict()
